...
`tryAdvance` to pull grace-period progress from the mutation path (RCU-DEC-005). Every iteration
also publishes `iteration + 1` to a padded per-CPU heartbeat slot, which the P4-DEC-008 watchdog
samples on a timer.
