...
|---|---|---|---|---|---|
| P4-ITEM-001 | Open | No | | Does this workload actually drive `reclaimSlabPage`, or does it merely *happen* to expose stale mappings by another route? | The spike found the P1-DEC-018 bug within ~1 iteration, which is *earlier* than a reclaim-driven explanation predicts — pages should still be fresh from the allocator that early. So the stale mapping likely comes from arena reuse at boot, not from reclaim. Until this is instrumented, do not claim the DEC-047 reclaim path itself is covered. |
