...
| ITEM-021 | Resolved 2026-08-01 → RCU-DEC-042 (Phase 4 shipped and `complete`) | | | The kernel-side mechanisms of the Phase-2 veneer have never executed in a boot, and neither Phase 2's harness nor Phase 3's torture suite can execute them. What closes that gap, and when? | Recorded at Phase-2 implementation, 2026-08-01. Phase 2 ships with no consumer, so a real boot exercises `Domain::init()` and nothing else — no `ReadGuard`, no `retire`, no sweep. Phase 3 is explicitly userspace-only (its own first Non-Goal defers in-kernel stress "to a later phase, planned separately"). So after Phase 3 is green, these remain unexecuted on a real target: the real `arch::InterruptDisabler` in the RCU-DEC-024 masked window; `KernelRcuHooks::onPreTouch` → `VMSubstrate::ensureTLBEntryFresh`; `SafePtr` dereference and its freshness cost; the real `assert` → `PANIC` macro on every debug check; and the `.icd` ordering against a live AP. **Why this is the sharpest of those, not a formality:** `onPreTouch` exists specifically to guard the vmsmalloc DEC-047 stale-TLB bug class, and DEC-047 was found by the *in-kernel* `VmsmallocStress` in QEMU — the userspace vmsmalloc harness did not and structurally could not find it, because the mock `ensureTLBEntryFresh` is a no-op and userspace has no page tables. RCU's exposure is the same shape (a drainer dereferencing intrusive links in another CPU's reclaimed slab pages under RCU-DEC-006 stealing), so the one Phase-2 mechanism most likely to be wrong is precisely the one the mocks cannot test. Options: an in-kernel `RcuStress` component modelled on `VmsmallocStress` (the precedent that caught DEC-047, and the one that would exercise stealing across real arenas); or fold the validation into the first real consumer (RadixVM) and accept that a stale-TLB defect surfaces as a consumer bug rather than a framework bug. Do not treat Phase 3 green as "the framework is validated on the target" — its Consumer Contract says "usable by any consumer", and this item is the boundary of that claim. |
...
| RCU-DEC-042 | Settled | **ITEM-021 is closed by Phase 4, which is shipped and `complete`.** Every mechanism ITEM-021 named as unexecuted now runs on the target under sustained multi-CPU load: the real `arch::InterruptDisabler` in the RCU-DEC-024 masked window, `KernelRcuHooks::onPreTouch` -> `VMSubstrate::ensureTLBEntryFresh`, `SafePtr` dereference, the real `assert` -> `PANIC`, and `.icd` ordering against live APs. `synchronize` and `barrier` were added in P4-DEC-009 and were the last two entry points with no in-kernel caller. | The gap ITEM-021 described was real and it was not a formality: **the very first boot of the Phase-4 spike found a shipped Phase-2 defect** (P1-DEC-018 — `onPreTouch` covered reads but not writes), in exactly the mechanism ITEM-021 predicted was most likely wrong, and for exactly the reason it gave — the mocks cannot model page tables, so the userspace suite structurally could not see it. Phase 4 then went further than the item asked: the DEC-047 hazard is **measured** as genuinely presented rather than argued (P4-DEC-006: ~14% of `RetireHead` accesses go through a mapping that CPU had stale); the target grew a **machine-checkable verdict** (P4-DEC-007, exit 0/3/5/7 — before which every in-kernel test the project ever had was unfalsifiable from outside); a **hang detector** (P4-DEC-008, exit 9) so a livelock stops reading as success; and the retire-path freshness cost is **quantified** (P4-DEC-010, ~59% of retire's instructions). What ITEM-021 warned against — reading Phase 3 green as "validated on the target" — no longer applies: it is validated on the target, with the caveat Phase 4's own first Hazard keeps, that absence of a loud failure is still a weak signal. Decided 2026-08-01. |
